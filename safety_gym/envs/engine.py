...
# Groups are a mujoco-specific mechanism for selecting which geom objects to
# "see" We use these for raycasting lidar, where there are different lidar
# types.  These work by turning "on" the group to see and "off" all the other
# groups.  See obs_lidar_natural() for more.
GROUP_GOAL = 0
GROUP_BOX = 1
GROUP_BUTTON = 1
GROUP_WALL = 2
...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        # Maximum number of environment steps in an episode
        'num_steps': 1000,
        # Magnitude of independent per-component gaussian action noise
        'action_noise': 0.0,

        # Placement limits (min X, min Y, max X, max Y)
        'placements_extents': [-2, -2, 2, 2],
        # Additional margin added to keepout when placing objects
        'placements_margin': 0.0,

        # Floor: In display mode, the visible part of the floor is cropped
        ...
        # If false, set the random seed before layout to constant
        ...
        'observe_pillars': False,  # Lidar observation of pillar objects
        'observe_buttons': False,  # Lidar observation of button objects
        'observe_gremlins': False,  # Gremlins are observed in lidar-like space
        ...
        # Size (width, height) of vision observation; gets flipped internally
        # to (rows, cols) format
        'vision_size': (60, 40),
        ...
        # Maximum distance for lidar sensing (if None, exponential distance)
        'lidar_max_dist': None,
        # Scaling factor for distance in exponential distance lidar
        'lidar_exp_gain': 1.0,
        ...
        # Within box_null_dist * box_size radius of box, no box reward given
        'box_null_dist': 2,

        # Reward is distance towards goal plus a constant for being within
        # range of goal reward_distance should be positive to encourage moving
        # towards the goal if reward_distance is 0, then the reward function is
        # sparse
        # Dense reward multiplied by the distance moved to the goal
        'reward_distance': 1.0,
        'reward_goal': 1.0,  # Sparse reward for being inside the goal area
        # Dense reward for moving the robot towards the box
        'reward_box_dist': 1.0,
        ...
        # What body to get orientation from
        'reward_orientation_body': 'robot',
        # Reward when encoutering a mujoco exception
        ...
        'reward_circle': 1e-1,
        # Clip reward, last resort against physics errors causing magnitude
        # spikes
        'reward_clip': 10,
        ...
        # Cost for pressing the wrong button, if constrain_buttons
        'buttons_cost': 1.0,
        ...
        'sensors_obs': ['accelerometer', 'velocimeter', 'gyro',
                        'magnetometer'],
        # Observe named joint position / velocity sensors
        'sensors_hinge_joints': True,
        # Observe named balljoint position / velocity sensors
        'sensors_ball_joints': True,
        # Observe sin/cos theta instead of theta
        'sensors_angle_components': True,

        # Walls--barriers in the environment not associated with any constraint
        # NOTE: this is probably best to be auto-generated rather than manually
        # specified
        ...
        # By default, no constraints are enabled, and all costs are indicator
        # functions.
        # Constrain robot from being in hazardous areas
        ...
        # Experimentally measured, based on size and density,
        # how far vases "sink" into the floor.
        'vases_sink': 4e-5,
        # Mujoco has soft contacts, so vases slightly sink into the floor, in a
        # way which can be hard to precisely calculate (and varies with time)
        ...
        # Threshold for displacement being "real"
        ...
        # Threshold for cost for being too close
        'gremlins_dist_threshold': 0.2,
        'gremlins_dist_cost': 1.0,  # Cost for being within distance threshold

        # Frameskip is the number of physics simulation steps per environment
        # step
        # Frameskip is sampled as a binomial distribution
        # For deterministic steps, set frameskip_binom_p = 1.0 (always take max
        # frameskip)
    ...
    def __init__(self, render_mode=None, camera_id=None, camera_name=None,
                 config={}):
        # First, parse configuration. Important note: LOTS of stuff happens in
        # parse, and many attributes of the class get set through setattr. If
        # you are trying to track down where an attribute gets initially set,
        # and can't find it anywhere else, it's probably set via the config
        # dict and this parse function.
        self.parse(config)
        gym.utils.EzPickle.__init__(self, config=config)

        # Load a simulation of the robot, just to figure out observation space
        self.robot = Robot(self.robot_base)

        self.action_space = \
            gym.spaces.Box(-1, 1, (self.robot.nu,), dtype=np.float32)
    ...
    def get_body_xpos(self, name):
        ...
        return self.data.xpos[i]

    def get_body_xvelp(self, name):
        i = self.model.body(name).id
        jacp = np.zeros((3, self.model.nv))
        mujoco.mj_jacBody(self.model, self.data, jacp=jacp, jacr=None, body=i)
        jacp = jacp.reshape((3, self.model.nv))
        return np.dot(jacp, self.data.qvel)

    def get_body_xquat(self, name):
        i = self.model.body(name).id
        return self.data.xquat[i]

    @property
    def robot_pos(self):
        ''' Helper to get current robot position '''
        return self.get_body_xpos('robot').copy()

    @property
    def goal_pos(self):
        ''' Helper to get goal position from layout '''
        if self.task in ['goal', 'push']:
            return self.get_body_xpos('goal').copy()
        elif self.task == 'button':
            return self.get_body_xpos(f'button{self.goal_button}').copy()
    ...
    def build_observation_space(self):  # noqa: C901
        ...
        if self.observe_freejoint:
            obs_space_dict['freejoint'] = \
                gym.spaces.Box(-np.inf, np.inf, (7,), dtype=np.float32)
        if self.observe_com:
            obs_space_dict['com'] = \
                gym.spaces.Box(-np.inf, np.inf, (3,), dtype=np.float32)
        if self.observe_sensors:
            for sensor in self.sensors_obs:  # Explicitly listed sensors
                dim = self.robot.sensor_dim[sensor]
                obs_space_dict[sensor] = \
                    gym.spaces.Box(-np.inf, np.inf, (dim,), dtype=np.float32)
            # Velocities don't have wraparound effects that rotational
            # positions do. Wraparounds are not kind to neural networks
            # Whereas the angle 2*pi is very close to 0, this isn't true in the
            # network. In theory the network could learn this, but in practice
            # we simplify it when the sensors_angle_components switch is
            # enabled.
            ...
            for sensor in self.robot.ballangvel_names:
                obs_space_dict[sensor] = \
                    gym.spaces.Box(-np.inf, np.inf, (3,), dtype=np.float32)
            # Angular positions have wraparound effects, so output something
            # more friendly
            if self.sensors_angle_components:
                ...
                # Since for angles, small perturbations in angle give small
                #  differences in sin/cos
                for sensor in self.robot.hinge_pos_names:
                    obs_space_dict[sensor] = \
                        gym.spaces.Box(-np.inf, np.inf, (2,), dtype=np.float32)
                # Quaternions are turned into 3x3 rotation matrices.
                # Quaternions have a wraparound issue in how they are
                # normalized, where the convention is to change the sign so the
                # first element to be positive.
                # If the first element is close to 0, this can mean small
                # differences in rotation lead to large differences in value as
                # the latter elements change sign.
                # This also means that the first element of the quaternion is
                # not expectation zero.
                # The SO(3) rotation representation would be a good replacement
                # here, since it smoothly varies between values in all
                # directions (the property we want), but right now we have very
                # little code to support SO(3) roatations.
                # Instead we use a 3x3 rotation matrix, which if normalized,
                # smoothly varies as well.
                for sensor in self.robot.ballquat_names:
                    obs_space_dict[sensor] = \
                        gym.spaces.Box(-np.inf, np.inf, (3, 3),
                                       dtype=np.float32)
            else:
        ...
        if self.task == 'push':
            if self.observe_box_comp:
                obs_space_dict['box_compass'] = \
                    gym.spaces.Box(-1.0, 1.0, (self.compass_shape,),
                                   dtype=np.float32)
            if self.observe_box_lidar:
                obs_space_dict['box_lidar'] = \
                    gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                                   dtype=np.float32)
        if self.observe_goal_dist:
            obs_space_dict['goal_dist'] = \
                gym.spaces.Box(0.0, 1.0, (1,), dtype=np.float32)
        if self.observe_goal_comp:
            obs_space_dict['goal_compass'] = \
                gym.spaces.Box(-1.0, 1.0, (self.compass_shape,),
                               dtype=np.float32)
        if self.observe_goal_lidar:
            obs_space_dict['goal_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.task == 'circle' and self.observe_circle:
            obs_space_dict['circle_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.observe_remaining:
            obs_space_dict['remaining'] = \
                gym.spaces.Box(0.0, 1.0, (1,), dtype=np.float32)
        if self.walls_num and self.observe_walls:
            obs_space_dict['walls_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.observe_hazards:
            obs_space_dict['hazards_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.observe_vases:
            obs_space_dict['vases_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.gremlins_num and self.observe_gremlins:
            obs_space_dict['gremlins_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.pillars_num and self.observe_pillars:
            obs_space_dict['pillars_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.buttons_num and self.observe_buttons:
            obs_space_dict['buttons_lidar'] = \
                gym.spaces.Box(0.0, 1.0, (self.lidar_num_bins,),
                               dtype=np.float32)
        if self.observe_qpos:
            obs_space_dict['qpos'] = \
                gym.spaces.Box(-np.inf, np.inf, (self.robot.nq,),
                               dtype=np.float32)
        if self.observe_qvel:
            obs_space_dict['qvel'] = \
                gym.spaces.Box(-np.inf, np.inf, (self.robot.nv,),
                               dtype=np.float32)
        if self.observe_ctrl:
            obs_space_dict['ctrl'] = \
                gym.spaces.Box(-np.inf, np.inf, (self.robot.nu,),
                               dtype=np.float32)
        if self.observe_vision:
            raise RuntimeError("Vision observations not supported")
            width, height = self.vision_size
            rows, cols = height, width
            self.vision_size = (rows, cols)
            obs_space_dict['vision'] = \
                gym.spaces.Box(0, 1.0, self.vision_size + (3,),
                               dtype=np.float32)
        # Flatten it ourselves
        self.obs_space_dict = obs_space_dict
        if self.observation_flatten:
            self.obs_flat_size = \
                sum([np.prod(i.shape) for i in self.obs_space_dict.values()])
            self.observation_space = \
                gym.spaces.Box(-np.inf, np.inf, (self.obs_flat_size,),
                               dtype=np.float32)
        else:
    ...
    def placements_dict_from_object(self, object_name):
        ...
        if hasattr(self, object_name + 's_num'):  # Objects with multiplicity
            ...
            object_placements = \
                getattr(self, plural_name + '_placements', None)
    ...
    def build_placements_dict(self):
        ...
        # Dictionary is map from object name -> tuple of
        # (placements list, keepout)
        placements = {}
    ...
    def draw_placement(self, placements, keepout):
        ...
        around it, where that keepout space overlaps with the placement
        rectangle.

        To sample an (x,y) pair, first randomly select which placement
        rectangle to sample from, where the probability of a rectangle is
        weighted by its area. If the rectangles are disjoint, there's an equal
        chance the (x,y) location will wind up anywhere in the placement space.
        If they overlap, then overlap areas are double-counted and will have
        higher density. This allows the user some flexibility in building
        placement distributions. Finally, randomly draw a uniform point within
        ...
        if placements is None:
            ...
            for placement in placements:
                xmin, ymin, xmax, ymax = \
                    self.constrain_placement(placement, keepout)
        ...
        return np.array([self.rs.uniform(xmin, xmax),
                         self.rs.uniform(ymin, ymax)])

    def random_rot(self):
        ''' Use internal random state to get a random rotation in radians '''
        return self.rs.uniform(0, 2 * np.pi)

    def build_world_config(self):  # noqa: C901
        ...
        #     world_config['render_context'] = -1
        # Hijack this so we don't create context
            ...
            for i in range(self.vases_num):
                ...
                object = {'name': name,
                          ...
                          'pos': np.r_[self.layout[name],
                                       self.vases_size - self.vases_sink],
                          'rot': self.random_rot(),
                          'group': GROUP_VASE,
                          'rgba': COLOR_VASE}
        ...
        if self.task in ['goal', 'push']:
            geom = {'name': 'goal',
                    ...
                    'pos': np.r_[self.layout['goal'],
                                 self.goal_size / 2 + 1e-2],
                    'rot': self.random_rot(),
                    'type': 'cylinder',
                    'contype': 0,
                    'conaffinity': 0,
        ...
        if self.hazards_num:
            for i in range(self.hazards_num):
                ...
                geom = {'name': name,
                        ...
                        # 'size': self.hazards_size / 2],
                        'pos': np.r_[self.layout[name], 2e-2],
                        # 'pos': self.hazards_size / 2 + 1e-2],
                        'rot': self.random_rot(),
                        'type': 'cylinder',
                        'contype': 0,
                        'conaffinity': 0,
                        'group': GROUP_HAZARD,
                        'rgba': COLOR_HAZARD * [1, 1, 1, 0.25]}  # 0.1]}
    ...
    def sample_goal_position(self):
        '''
        Sample a new goal position and return True, else False if sample
        rejected
        '''
    ...
    def build_goal_position(self):
        ...
        self.world_config_dict['geoms']['goal']['pos'][:2] = \
            self.layout['goal']
    ...
    '''
        ...
        return np.sqrt(np.sum(np.square(self.box_pos -
    ...
    def obs_lidar_natural(self, group):
        ...
        grp = np.asarray([i == group for i in range(int(mujoco.mjNGROUP))],
                         dtype='uint8')
    ...
    def obs_lidar_pseudo(self, positions):
        ...
        The detection directions are exclusive and exhaustive for a full 360
        view.  Each bin reads 0 if there are no objects in that direction.  If
        there are multiple objects, the distance to the closest one is used.
        Otherwise the bin reads the fraction of the distance towards the robot.

        E.g. if the object is 90% of lidar_max_dist away, the bin will read
        0.1, and if the object is 10% of lidar_max_dist away, the bin will read
        0.9.  (The reading can be thought of as "closeness" or inverse
        distance)
    ...
    def obs(self):  # noqa: C901
        ...
        if self.observe_freejoint:
            joint_id = self.model.joint('robot').id
        ...
        if self.observe_remaining:
            ...
            assert 0.0 <= obs['remaining'][0] <= 1.0, \
                'bad remaining {}'.format(obs['remaining'])
        if self.walls_num and self.observe_walls:
            obs['walls_lidar'] = self.obs_lidar(self.walls_pos, GROUP_WALL)
        if self.observe_hazards:
            obs['hazards_lidar'] = self.obs_lidar(self.hazards_pos,
                                                  GROUP_HAZARD)
        if self.observe_vases:
            obs['vases_lidar'] = self.obs_lidar(self.vases_pos, GROUP_VASE)
        if self.gremlins_num and self.observe_gremlins:
            obs['gremlins_lidar'] = self.obs_lidar(self.gremlins_obj_pos,
                                                   GROUP_GREMLIN)
        if self.pillars_num and self.observe_pillars:
            obs['pillars_lidar'] = self.obs_lidar(self.pillars_pos,
                                                  GROUP_PILLAR)
        if self.buttons_num and self.observe_buttons:
            # Buttons observation is zero while buttons are resetting
            if self.buttons_timer == 0:
                obs['buttons_lidar'] = self.obs_lidar(self.buttons_pos,
                                                      GROUP_BUTTON)
            else:
        ...
        if self.observation_flatten:
            ...
            for key in obs:
                obs[key] = np.float32(obs[key])
        assert self.observation_space.contains(obs), \
            f'Bad obs {obs} {self.observation_space}'
        return obs


    def cost(self):  # noqa: C901
        ...
        buttons_constraints_active = \
            self.constrain_buttons and (self.buttons_timer == 0)
        for contact in self.data.contact[:self.data.ncon]:
            geom_ids = [contact.geom1, contact.geom2]
            geom_names = sorted([self.model.geom(g).name
                                 for g in geom_ids])
            if self.constrain_vases and any(n.startswith('vase')
                                            for n in geom_names):
                if any(n in self.robot.geom_names for n in geom_names):
                    cost['cost_vases_contact'] += self.vases_contact_cost
            if self.constrain_pillars and any(n.startswith('pillar')
                                              for n in geom_names):
                if any(n in self.robot.geom_names for n in geom_names):
                    cost['cost_pillars'] += self.pillars_cost
            if buttons_constraints_active and any(n.startswith('button')
                                                  for n in geom_names):
                if any(n in self.robot.geom_names for n in geom_names):
                    if not any(n == f'button{self.goal_button}'
                               for n in geom_names):
                        cost['cost_buttons'] += self.buttons_cost
            if self.constrain_gremlins and any(n.startswith('gremlin')
                                               for n in geom_names):
                if any(n in self.robot.geom_names for n in geom_names):
        ...
        if self.constrain_vases and self.vases_velocity_cost:
            ...
            for i in range(self.vases_num):
                name = f'vase{i}'
                vel = np.sqrt(np.sum(np.square(
                    self.get_body_xvelp(name))))
                if vel >= self.vases_velocity_threshold:
                    cost['cost_vases_velocity'] += \
                        vel * self.vases_velocity_cost
        ...
        if self.constrain_hazards:
            ...
            for h_pos in self.hazards_pos:
                ...
                if h_dist <= self.hazards_size:
                    cost['cost_hazards'] += \
                        self.hazards_cost * (self.hazards_size - h_dist)
        ...
        if self.gremlins_num:  # self.constrain_gremlins:
            ...
            for i in range(self.gremlins_num):
                ...
                target = np.array([np.sin(phase), np.cos(phase)]) * \
                    self.gremlins_travel
                pos = np.r_[target, [self.gremlins_size]]
                body_id = self.model.body(name + 'mocap').id
    ...
    def update_layout(self):
        ...
        for k in list(self.layout.keys()):
            ...
            if 'gremlin' in k:
                continue
            self.layout[k] = self.get_body_xpos(k)[:2].copy()
    ...
    def step(self, action):
        ...
        if self.action_noise:
            self.data.ctrl[:] += \
                self.action_noise * self.rs.randn(self.model.nu)
        ...
        for _ in range(self.rs.binomial(self.frameskip_binom_n,
            ...
            except mujoco.FatalError as me:
        ...
        if exception:
            ...
            if self.goal_met():
                ...
                if self.continue_goal:
                    # Update the internal layout so we can correctly resample
                    # (given objects have moved)
                    self.update_layout()
                    # Reset the button timer (only used for task='button'
                    # environments)
                    ...
                    if self.terminate_resample_failure:
                        ...
                        # Try to make a goal, which could raise a
                        # ResamplingError exception
                        self.build_goal()
                else:
                    self.done = True
                    terminated = True
    ...
    def reward(self):
        ...
        if self.task == 'push':
            ...
            reward += \
                (self.last_dist_box - dist_box) * self.reward_box_dist * \
                gate_dist_box_reward
        ...
        if self.task == 'push':
            ...
            reward += \
                (self.last_box_goal - dist_box_goal) * self.reward_box_goal
        ...
        if self.task == 'circle':
            ...
            reward += \
                (((-u*y + v*x) / radius) /
                 (1 + np.abs(radius - self.circle_radius))) * \
                self.reward_circle
        # Intrinsic reward for uprightness
        if self.reward_orientation:
            zalign = quat2zalign(self.get_body_xquat(
    ...
    def render(self):
        ...
        # TODO: Fix camera_id, camera_name
        self.world.mujoco_renderer.render(
        ...
        #     if 'box_lidar' in self.obs_space_dict or \
        #             'box_compass' in self.obs_space_dict:
        #         if 'box_lidar' in self.obs_space_dict:
        #             self.render_lidar([self.box_pos], COLOR_BOX, offset,
        #                               GROUP_BOX)
        ...
        #     if 'goal_lidar' in self.obs_space_dict or \
        #             'goal_compass' in self.obs_space_dict:
        #         if 'goal_lidar' in self.obs_space_dict:
        #             self.render_lidar([self.goal_pos], COLOR_GOAL, offset,
        #                               GROUP_GOAL)
        ...
        #         self.render_lidar(self.buttons_pos, COLOR_BUTTON, offset,
        #                           GROUP_BUTTON)
        #         offset += self.render_lidar_offset_delta
        ...
        #         self.render_lidar(self.walls_pos, COLOR_WALL, offset,
        #                           GROUP_WALL)
        #         offset += self.render_lidar_offset_delta
        #     if 'hazards_lidar' in self.obs_space_dict:
        #         self.render_lidar(self.hazards_pos, COLOR_HAZARD, offset,
        #                           GROUP_HAZARD)
        #         offset += self.render_lidar_offset_delta
        #     if 'pillars_lidar' in self.obs_space_dict:
        #         self.render_lidar(self.pillars_pos, COLOR_PILLAR, offset,
        #                           GROUP_PILLAR)
        #         offset += self.render_lidar_offset_delta
        ...
        #         self.render_lidar(self.vases_pos, COLOR_VASE, offset,
        #                           GROUP_VASE)
        #         offset += self.render_lidar_offset_delta

        # # Add goal marker
        # if self.task == 'button':
        #     self.render_area(self.goal_pos, self.buttons_size * 2,
        #                      COLOR_BUTTON, 'goal', alpha=0.1)

        # # Add indicator for nonzero cost
        # if self._cost.get('cost', 0) > 0:
        #     self.render_sphere(self.world.robot_pos(), 0.25,
        #                        COLOR_RED, alpha=.5)
